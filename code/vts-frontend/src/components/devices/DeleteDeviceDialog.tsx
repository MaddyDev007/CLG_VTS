...
export function DeleteDeviceDialog({ device, isOpen, onClose, onSuccess }: DeleteDeviceDialogProps) {
    ...
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-slate-950/55 p-4'>
      <div className='w-full max-w-md rounded-2xl border border-white/30 bg-white p-5 shadow-2xl dark:border-slate-700 dark:bg-[#1e293b]'>
        <div className='mb-3 flex items-start gap-2'>
          ...
          <div>
            <h3 className='text-base font-semibold text-slate-900 dark:text-slate-100'>Delete Device</h3>
        ...
        <p className='rounded-lg bg-slate-100 px-3 py-2 text-sm text-slate-700 dark:bg-slate-800 dark:text-slate-200'>
          Device name: <span className='font-semibold'>{device.deviceId}</span>
        </p>
