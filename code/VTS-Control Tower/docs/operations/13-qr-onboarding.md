...
1. Manual provisioning
   ...
   - backend confirms device exists or creates a pending record
...
- QR onboarding may claim a pending discovered device, but tenant assignment must remain backend-authorized

## Current-State Note

There is no complete QR onboarding flow implemented in the current codebase. This document defines the target contract so frontend, backend, and firmware can converge on the same structure.
