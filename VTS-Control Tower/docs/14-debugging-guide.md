...
- same broker host/port across firmware, backend, `vts-device-simulator`, and simulator bridge
- simulator bridge `/devices` returns real assigned devices from DB
...
1. start backend infrastructure, usually from `vts-backend/docker-compose.yml`
2. start Nest backend
3. start frontend
4. start one simulator if needed

Current shared MQTT test choice:

- old-backend Mosquitto broker on port `1883`
- backend containers typically use `mqtt://mosquitto:1883` internally
- external firmware must target the host machine LAN/public IP or domain on port `1883`
- all publishers/subscribers must target that same broker instance for end-to-end validation
